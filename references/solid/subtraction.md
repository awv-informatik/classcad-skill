...
- **Consumed solid IDs are rejected with a clean error.** After a default subtraction (keepTools=false), the tool is deleted; referencing its ID in any later solid op (`solid.translation`, `solid.copy`, `solid.subtraction`, …) returns `maxLevel 51` with `"...has an invalid id!"` (code 1006) — the same as referencing an ID that never existed; the id-type validation rejects consumed IDs up front (no hang). Track which IDs are valid so you get the result you expect.
