...
- **Some renderers ignore stored color** — clients that assign their own per-body color palette will not show appearance changes. Color/transparency are stored in the model regardless; use `requestVisualisation` to verify the stored values.
